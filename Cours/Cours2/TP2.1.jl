...
@constraint(m, 2 * m[:x1]^2 + 3 * m[:x2] <= 12 ) # Contrainte 1
@constraint(m, m[:x1] - m[:x2] >= -4 )# Contrainte 2
@constraint(m, m[:x1] >= 4 )

@constraint(m, [i in 1:n], sum(m[:affectation][i,j] for j in 1:n) >= 5 ) # Contraintes pour chaque ligne et colonnes
...
m2 = get_model_1(h_interval)
JuMP.optimize!(m2)
objective_value(m2)

# Pour avoir une valeur de variable du modèle : value(nom_model[:nom_variable]) 
# Exemple :  value.(m2[:p_ch])   renvoi les valeurs de p_ch pour chaque pas de temps
...
# Ajout du dimmentionnement comme variable de décision au modèle
...
# 2 rendement différents en fonction du C-rate et calcul des coûts de dépassement 
# Le model est un modèle MILP
...
######################### Model 3 ##############################
################################################################
# Evolution de la capacité de la batterie à chaque heure ne fonction de son utilisation 
# Le modèle est MINLP
