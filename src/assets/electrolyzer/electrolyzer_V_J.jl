  ...
  mutable struct Electrolyzer_V_J <: AbstractElectrolyzer
    # Paramètres
    α_p::Float64
    η_E_H2::Float64
    η_E_H::Float64
    ...
    # Inner constructor
    Electrolyzer_V_J(; α_p = 8/100, #Min power 
            η_E_H2 = 0.5,
            η_H_H2 = 0.3,
            lifetime = 14,
            nHoursMax = 20000.,
            ...
            new(α_p, η_E_H2, η_H_H2, lifetime, nHoursMax, SoH_threshold, update_by_year, K, k_aux, bounds, soh_ini, V_J_ini, powerMax_ini, V_min, couplage)
  end
  ...
  ### Preallocation
  function preallocate!(elyz::Electrolyzer_V_J, nh::Int64, ny::Int64, ns::Int64)
...
### Operation dynamic
function compute_operation_dynamics!(h::Int64, y::Int64, s::Int64, elyz::Electrolyzer_V_J, decision::Float64, Δh::Int64)
    #Apply minimum power
    elyz.α_p * (elyz.powerMax[h,y,s]/(1 + elyz.k_aux)) <= -decision ? power_E = max(decision, -elyz.powerMax[h,y,s]) : power_E = 0. 
    
    
    if power_E < 0
        #Compute the remaining power after feeding the auxiliaries 
        P_elyz = ceil(power_E / (1 + elyz.k_aux); digits=6)

        #Find the corresponding current from an interpolation from P(I) curve 
        if elyz.couplage
            j = interpolation(elyz.V_J[3,:], elyz.V_J[1, :], -P_elyz, true)
        else
            P = elyz.V_J_ini.V .* (elyz.V_J_ini.J * elyz.surface * elyz.N_cell) 
            j = interpolation(P, elyz.V_J_ini.J, -P_elyz, true)
        end
        
        i = j * elyz.surface
        
        #Power_out/power_in (per cell)
        η_E_H2 = i * elyz.K / (-power_E/ncell) 
...
### Operation dynamic
function compute_operation_dynamics(elyz::Electrolyzer_V_J, h::Int64, y::Int64, s::Int64, decision::Float64, Δh::Int64)
    #Apply minimum power
    elyz.α_p * (elyz.powerMax[h,y,s]/(1 + elyz.k_aux)) <= -decision ? power_E = max(decision, -elyz.powerMax[h,y,s]) : power_E = 0. 
...
function update_elyz(elyz::Electrolyzer_V_J, powers::Vector{Float64})
    ...
    function initialize_investments!(s::Int64, elyz::Electrolyzer_V_J, decision::NamedTuple{(:surface, :N_cell), Tuple{Float64, Int64}})
        ...
        elyz.powerMax[1,1,s] = maximum(elyz.V_J[3,:]) * (1+elyz.k_aux)
  ...
  ### Investment dynamic
    function compute_investment_dynamics!(y::Int64, s::Int64, elyz::Electrolyzer_V_J,  decision::NamedTuple{(:surface, :N_cell), Tuple{Float64, Int64}})
            
        elyz.powerMax[1,y+1,s], elyz.soh[1,y+1,s], elyz.V_J = compute_investment_dynamics(elyz, (powerMax = elyz.powerMax[end,y,s], soh = elyz.soh[end,y,s]), decision)
    end


    function compute_investment_dynamics(elyz::Electrolyzer_V_J, state::NamedTuple{(:powerMax, :soh), Tuple{Float64, Float64}},  decision::NamedTuple{(:surface, :N_cell), Tuple{Float64, Int64}})
        if decision.N_cell > 1e-2 
            ...
            powerMax_next = maximum(elyz.V_J[3,:]) * (1+elyz.k_aux)
        ...
        return powerMax_next, soh_next, V_J
    end
