...
function compute_operation_soc_linear(liion::AbstractLiion, state::NamedTuple{(:Erated, :soc, :soh), Tuple{Float64, Float64, Float64}}, decision::Float64, Δh::Int64)
	...
	if liion.couplage.R
		η = η_ini - (0.2303 * (1-state.soh))   #redondo iglesias (15)
	else
		η = η_ini
	end
...
#Energy efficiency of lithium-ion battery used as energy storage devices in micro-grid,   Kaiyuan Li
 function compute_operation_soc_polynomial(liion::AbstractLiion, state::NamedTuple{(:Erated, :soc, :soh), Tuple{Float64, Float64, Float64}}, decision::Float64, Δh::Int64)
	...
	if decision >= 0 
		η_ini = 0.0033 * C_r ^ 2 - 0.0297 * C_r + 0.99814
	else
		η_ini =  0.002232 * C_r ^ 2 - 0.0246 * C_r + 1
	end

	

	if liion.couplage.R
		η = η_ini - (0.2303 * (1-state.soh))   #redondo iglesias (15) simplifié
	else
		η = η_ini
	end
 ...
 function compute_operation_soc_tremblay_dessaint(liion::AbstractLiion, state::NamedTuple{(:Erated, :soc, :soh), Tuple{Float64, Float64, Float64}}, V::Float64, decision::Float64, Δh::Int64)
	 ...
	 # But the influence of the Ohmic resistance is really low, their should be a way to modelize this differently
	 if liion.couplage.R
